...
for epoch in range(epochs):
    ...
    print(
        f"[{epoch + 1}/{epochs}] Training Accuracy:{100*train_correct/total} % Training Loss:{train_loss/len(train_set):.3f} time:{(time.time()-start)/60:.2f}")
    ...
    with torch.no_grad():
        ...
        # 輸出驗證數據
        print(
            f"[{epoch + 1}/{epochs}] Validation Accuracy:{100*val_correct/total_val} % Validation Loss:{val_loss/len(valid_set):.3f} time:{(time.time()-start)/60:.2f}")
...
# 儲存模型
torch.save(vgg19.state_dict(), './model/batch_40_lr0002_par.pt')
torch.save(vgg19, './model/batch_40_lr0002.pt')
...
vgg19_test.load_state_dict(torch.load(
    './model/batch_40_lr0002_par.pt'))
